...
int matchNames(char * str1,  char *str2)
{
    ...
    char strAUX[MAXCHAR];
    char delimiter='\n';
    int size=(unsigned)strlen(str1)-1;
    if (strchr(str1,' ') != NULL) delimiter=' ', size=(unsigned)strlen(str1)-2;

    strncpy(strAUX, str1, size);
    strAUX[size] = '\n'; // place the null terminator
    strAUX[size+1] = '\0'; // place the null terminator

    printf("%s", strAUX);
    while(*str1++ != delimiter && *str2++ != delimiter)//stop in the first blank space or new line
    {
        if(*str1 != *str2)
        {
